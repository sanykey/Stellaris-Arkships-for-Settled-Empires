...
Settled Waystations use a dedicated scripted capacity and do not consume Starbase Capacity. Their base capacity is **3**. Waystation technologies add **+2**, **+4** and **+6**; Mega-Engineering adds **+3**; Stellar Expansion and Manifest Destiny add **+2** each; Grasp the Void adds **+5**; adopting Unyielding and taking Fortress Doctrine add **+2** each; and Galactic Ambition increases the resulting capacity by **+50%**. A country modifier displays current usage and a breakdown of these sources. Completed Waystations of every tier and Waystations under construction share this limit. Nomadic Waystations continue to use Starbase Capacity and retain all vanilla technology effects.

The vanilla Waystation Pact remains unchanged for every pairing involving a Nomadic empire. Two Settled empires may also form the pact if either empire has **Two Paths, One Destiny**. A Settled empire with the perk may construct its special Waystations in the other Settled empire's territory only while the pact exists and that empire's borders are open.

Enforcing closed borders dismantles foreign Settled Waystations inside the enforcing empire. The same check is performed when a system changes owner: a Settled Waystation is dismantled if the new owner has closed its borders to the station's owner. If the borders remain open, the station remains; without a valid Waystation Pact it is treated as an unlicensed Waystation by the vanilla opinion system.

Settled Waystations can observe pre-FTL civilizations in their systems when no Observation Post is present. They receive the vanilla Waystation observation events and contribute **0.5 monthly progress per eligible pre-FTL civilization** to the shared Observation Insights situation. This contribution is added separately to the progress generated by ordinary Observation Posts.
...
The standard Logistics Ship has a separately configurable **Deliver Resources to Colonies** automation setting, enabled by default. It may deliver its stockpile either to an owned Arkship through the vanilla action or directly to a nearby owned colony through the AFSE action. Colony targets are searched within **10 hyperlane jumps**, and delivery transfers the cargo into the country stockpile. Both destination actions use the same cargo-fill weights; automation does not inherently prioritize colonies over Arkships.

The Eager Explorer **Logistics Vessel** retains its vanilla scripted-action set and currently does not receive direct delivery to colonies.
...
- `common/starbase_levels/05_waystation_levels.txt`: separate AFSE counterparts of the three Waystation levels reuse the vanilla Waystation ship sizes but consume no Starbase Capacity;
- `common/scripted_triggers/02_scripted_triggers_first_contact_dlc.txt`: `pre_ftl_in_age_has_nomad_waystation_observer` and `pre_ftl_has_nomad_waystation_observer`;
...
- `events/observation_events.txt`: full relative-path override enabling settled AFSE Waystations to participate in the incoming-asteroid pre-FTL event;
- `events/nomads_events_1.txt`: `nomads.4720`.

Objects with vanilla IDs are overridden by identifier. The Arkship inline script and Waystation megastructure file are full overrides at their original relative paths. The mod also adds `afse` events, scripted actions, triggers, effects, values, modifiers and on-actions for settlement, colonization, migration, Settled Arkship Empire Size accounting, Settled Waystation capacity and access enforcement, pre-FTL observation, and Logistics Ship delivery to colonies.
